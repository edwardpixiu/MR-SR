...
def get_num_groups(num_channels, preferred_groups=8):
    """
    ...
    Args:
        ...
        preferred_groups (int): Preferred number of groups (default: 8)

    Returns:
        int: Number of groups that evenly divides num_channels
    """
    if num_channels < preferred_groups:
        # If fewer channels than preferred groups, use fewer groups
        return gcd(num_channels, preferred_groups)

    # Find the largest divisor of num_channels that's <= preferred_groups
    for num_groups in range(preferred_groups, 0, -1):
...
class DoubleConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, mid_channels=None, dilation=1):
        ...
        self.double_conv = nn.Sequential(
            ...
            nn.GroupNorm(num_groups=get_num_groups(mid_channels), num_channels=mid_channels),
            ...
            nn.GroupNorm(num_groups=get_num_groups(out_channels), num_channels=out_channels),
            nn.LeakyReLU(negative_slope=0.2, inplace=True)
        )
...
class Up(nn.Module):
    ...
    def __init__(self, in_ch_up, in_ch_skip, out_channels):
        ...
        self.up = nn.Sequential(
            ...
            nn.GroupNorm(num_groups=get_num_groups(in_ch_up // 2), num_channels=in_ch_up // 2),
            nn.LeakyReLU(negative_slope=0.2, inplace=True)
        )
...
class PixelShuffleUp(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, scale_factor=2):
        ...
        self.norm = nn.GroupNorm(num_groups=get_num_groups(out_channels), num_channels=out_channels)
...
class UNetSuperRes(nn.Module):
    ...
    def __init__(self, in_channels=1, out_channels=1, base_filters=32, initial_alpha=0.0):
        ...
        self.final_up_bilinear = nn.Sequential(
            ...
            nn.GroupNorm(num_groups=get_num_groups(f // 2), num_channels=f // 2),
            nn.LeakyReLU(negative_slope=0.2, inplace=True)
        )
