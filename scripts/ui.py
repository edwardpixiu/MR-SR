...
class MRIUI:
    def __init__(self, stdscr):
        ...
        self.params = {
            ...
            "n_slices_extract": 100,  # Increased from 10 - more data = better results
            "lower_percent": 0.2,
            "upper_percent": 0.8,
            ...
            "learning_rate": 1e-4,
            "weight_decay": 1e-5,
            "ssim_weight": 0.4,  # Increased from 0.3 - better perceptual quality
            "perceptual_weight": 0.1,  # Increased from 0.0 - adds VGG perceptual loss
            "initial_alpha": 0.0,  # Initial blending weight for bilinear vs pixelshuffle
            "vgg_layer_idx": 35,
            "perceptual_loss_type": 'l1',
            "validation_split": 0.2,
            "patience": 15,  # Increased from 10 - more patience for convergence
            "num_workers": get_optimal_workers(),  # Set based on system capabilities
            "seed": 42,  # Fixed seed (changed from random) for reproducibility
            "augmentation": True,  # Changed from False - augmentation improves generalization
            "use_tensorboard": True,  # Changed from False - essential for tracking experiments
